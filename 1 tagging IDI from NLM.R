...
# Creating blank project (do it only once)
...
# Read your raw transcript text file
transcript_path <- file.path(path_doc,
                             "Transkrip_IDI 1_JKT_IT_Sewa Mobil.txt")
transcript_text <- readLines(transcript_path, encoding = "UTF-8", warn = FALSE)
...
############################ [TRY AGAIN 3-LEVELS] Autotagging FGD 1 #################################
...
# Load codes
codes <- read.csv(file.path(path_code, "Gemini code snippet - FGD 1.csv"))

# Load transcript (txt)
transcript_path <- file.path(
  path_doc,
  "Transkrip_FGD 1_JKT_4W_M.txt"
)
transcript_text <- readLines(transcript_path, encoding = "UTF-8", warn = FALSE)
transcript_text <- paste(transcript_text, collapse = "  ")
transcript_text <- normalize_text(transcript_text)
...
# Write output
path_tagged <- file.path(path_doc, "Tagged transcript_FGD 1.txt")
writeLines(working_transcript, path_tagged, useBytes = TRUE)

# Save log so you know what happened
write.csv(log_results, file.path(path_memo, "Tagging log_FGD 1.csv"), row.names = FALSE)
